...
////////////////////////////////////////////////////////////////////////////////
// Testbench: tb_fp4e1m2_fp8e4m3_mac
////////////////////////////////////////////////////////////////////////////////
module tb_fp4e1m2_fp8e4m3_mac;
  ...
  function [7:0] fp4e1m2_to_fp8e4m3;
    input [3:0] fp4;
    reg sign;
    reg exp_bit;
    reg [1:0] frac2;
    begin
      ...
      exp_bit = fp4[2];
      frac2   = fp4[1:0];
      if (exp_bit == 1'b1) begin
        fp4e1m2_to_fp8e4m3 = QNAN8;
      end else if (frac2 == 2'b00) begin
  ...
  function [3:0] pack_fp4;
    input integer sign;
    input integer exp;
    input integer frac;
    begin
      pack_fp4 = {sign[0], exp[0], frac[1:0]};
    end
  endfunction
  ...
  initial begin
    ...
    fp4_values[1] = 4'b0001; // +0.25
    fp4_values[2] = 4'b0010; // +0.50
    fp4_values[3] = 4'b0011; // +0.75
    fp4_values[4] = 4'b0101; // -0.25
    fp4_values[5] = 4'b0110; // -0.50
    fp4_values[6] = 4'b1001; // +0.25 (sign=1? actual negative? need more)
    fp4_values[7] = 4'b1111; // NaN

    b_choices[0] = 8'h38; // +1
    b_choices[1] = 8'hB8; // -1
    b_choices[2] = 8'h30; // +0.5
    b_choices[3] = 8'hC0; // -2

    c_choices[0] = PZERO8;
    c_choices[1] = 8'h38; // +1
    c_choices[2] = 8'hB8; // -1
    c_choices[3] = 8'h30; // +0.5
    ...
    bvec[0] = pack2_8(8'h38, 8'h30);
    cvec[0] = pack4_8(8'h38,8'hB8,8'h30,8'h00);

    avec[1] = {4'b1001,4'b0101};
    ...
    for (idx = 0; idx < N; idx = idx + 1) begin
      a1_fp8 = fp4e1m2_to_fp8e4m3(avec[idx][3:0]);
      a2_fp8 = fp4e1m2_to_fp8e4m3(avec[idx][7:4]);
  ...
  initial begin
    ...
    $display("\n--- FP4(E1M2) x FP8(E4M3) MAC (LAT=%0d, Vectors=%0d) ---", LAT, N);
