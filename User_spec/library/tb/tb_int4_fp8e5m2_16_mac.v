...
////////////////////////////////////////////////////////////////////////////////
// Testbench: tb_int4_fp8e5m2_16_mac
//   - INT4 × FP8(E5M2) multiply, accumulate into FP16 lanes
////////////////////////////////////////////////////////////////////////////////
module tb_int4_fp8e5m2_16_mac;
  ...
  function [7:0] int4_to_fp8e5m2_model; input [3:0] val4;
    ...
    begin
      ...
      if (sval == 0) begin
        ...
        endcase
        case (msb_idx)
          3'd3: mant3 = {absval[3], absval[2], absval[1]};
          3'd2: mant3 = {absval[2], absval[1], absval[0]};
          3'd1: mant3 = {absval[1], absval[0], 1'b0};
          default: mant3 = {absval[0], 2'b00};
        endcase
  ...
  function [7:0] fp8e5m2_mul_lane_model; input [7:0] a; input [7:0] b;
    reg sa,sb; reg [4:0] ea,eb; reg [1:0] fa,fb;
    reg a_nan,a_inf,a_zero,b_nan,b_inf,b_zero;
    reg signp; integer esum; reg [2:0] Ma,Mb; reg [5:0] prod; reg carry; reg [1:0] frac2;
    begin
  ...
  function [15:0] fp8e5m2_to_fp16; input [7:0] val;
    reg sign; reg [4:0] exp8; reg [1:0] frac2;
    begin
      sign = val[7]; exp8 = val[6:2]; frac2 = val[1:0];
      if (exp8 == 5'h1F) begin
  ...
  function [15:0] fp16_add_ref; input [15:0] a; input [15:0] b;
    reg sign_a, sign_b; reg [4:0] exp_a, exp_b; reg [9:0] frac_a, frac_b;
    reg [10:0] mant_a, mant_b; reg zero_a, zero_b; reg [4:0] exp_big, exp_small, exp_res;
    reg [10:0] mant_big, mant_small; reg sign_big, sign_small, sign_res;
    reg [12:0] mant_big_ext, mant_small_ext, mant_small_shifted, mant_sum;
    integer diff; integer shift; reg [15:0] res;
    begin : add_fn
      if (fp16_is_nan(a)) begin res = a[9:0] ? a : FP16_QNAN; fp16_add_ref = res; disable add_fn; end
      if (fp16_is_nan(b)) begin res = b[9:0] ? b : FP16_QNAN; fp16_add_ref = res; disable add_fn; end
      if (fp16_is_inf(a) && fp16_is_inf(b) && (a[15]!=b[15])) begin fp16_add_ref = FP16_QNAN; disable add_fn; end
      if (fp16_is_inf(a)) begin fp16_add_ref = {a[15],5'h1F,10'h000}; disable add_fn; end
      if (fp16_is_inf(b)) begin fp16_add_ref = {b[15],5'h1F,10'h000}; disable add_fn; end

      sign_a=a[15]; sign_b=b[15]; exp_a=a[14:10]; exp_b=b[14:10]; frac_a=a[9:0]; frac_b=b[9:0];
      zero_a = fp16_is_zero_or_sub(a); zero_b = fp16_is_zero_or_sub(b);
  ...
  initial begin
    ...
    b_choices[3] = FP8_P1_5;
    b_choices[4] = FP8_P2_0;
    b_choices[5] = FP8_N2_0;
    ...
    bvec[0]  = pack2_8(FP8_P0_5, FP8_N3_0);
    cvec8[0] = pack4_8(8'h00, FP8_P1_0, FP8_N1_0, 8'h00);

    avec[1]  = {int_to_int4(-1), int_to_int4(3)};
    bvec[1]  = pack2_8(FP8_P3_0, FP8_P1_0);
    cvec8[1] = pack4_8(FP8_P2_0, FP8_P0_5, FP8_N0_5, FP8_P1_0);

    avec[2]  = {int_to_int4(0), int_to_int4(0)};
    bvec[2]  = pack2_8(FP8_P1_0, FP8_P1_5);
    cvec8[2] = pack4_8(FP8_N1_0, FP8_P1_0, FP8_N1_0, FP8_P1_0);

    avec[3]  = {int_to_int4(-8), int_to_int4(-8)};
    bvec[3]  = pack2_8(PINF8, PINF8);
    cvec8[3] = pack4_8(PINF8, PINF8, PINF8, PINF8);

    for (idx = 0; idx < N; idx = idx + 1) begin
      cvec16[idx] = pack4_16(
          fp8e5m2_to_fp16(cvec8[idx][31:24]),
          fp8e5m2_to_fp16(cvec8[idx][23:16]),
          fp8e5m2_to_fp16(cvec8[idx][15:8]),
          fp8e5m2_to_fp16(cvec8[idx][7:0])
      );
    end
  ...
  initial begin
    ...
    $display("\n--- INT4 × FP8(E5M2) → FP16 MAC (LAT=%0d, Vectors=%0d) ---", LAT, N);
