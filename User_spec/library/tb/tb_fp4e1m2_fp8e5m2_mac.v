...
////////////////////////////////////////////////////////////////////////////////
// Testbench: tb_fp4e1m2_fp8e5m2_mac
////////////////////////////////////////////////////////////////////////////////
module tb_fp4e1m2_fp8e5m2_mac;
  ...
  function [7:0] fp4e1m2_to_fp8e5m2;
    input [3:0] fp4;
    reg sign;
    reg exp_bit;
    reg [1:0] frac2;
    begin
      ...
      exp_bit = fp4[2];
      frac2   = fp4[1:0];
      if (exp_bit == 1'b0 && frac2 == 2'b00) begin
        fp4e1m2_to_fp8e5m2 = {sign, 7'd0, 1'b0};
      end else begin
        // Normalised value (exp=1) -> exponent 15
        fp4e1m2_to_fp8e5m2 = {sign, 5'd15, frac2};
      end
    end
  endfunction
  ...
  initial begin
    ...
    fp4_values[3] = 4'b0011;
    fp4_values[4] = 4'b0101;
    fp4_values[5] = 4'b0110;
    fp4_values[6] = 4'b1001;
    ...
    for (idx = 0; idx < N; idx = idx + 1) begin
      a1_fp8 = fp4e1m2_to_fp8e5m2(avec[idx][3:0]);
      a2_fp8 = fp4e1m2_to_fp8e5m2(avec[idx][7:4]);
  ...
  initial begin
    ...
    $display("\n--- FP4(E1M2) x FP8(E5M2) MAC (LAT=%0d, Vectors=%0d) ---", LAT, N);
