...
////////////////////////////////////////////////////////////////////////////////
// Testbench: tb_fp4e2m1_fp8e5m2_mac
////////////////////////////////////////////////////////////////////////////////
module tb_fp4e2m1_fp8e5m2_mac;
  ...
  function [7:0] fp4e2m1_to_fp8e5m2;
    input [3:0] fp4;
    reg sign;
    reg [1:0] exp2;
    reg frac1;
    ...
    begin
      ...
      frac1 = fp4[0];
      if (exp2 == 2'b11) begin
        fp4e2m1_to_fp8e5m2 = (frac1 == 1'b0) ? {sign, 5'h1F, 2'b00} : QNAN8;
      end else if (exp2 == 2'b00) begin
        fp4e2m1_to_fp8e5m2 = (frac1 == 1'b0) ? {sign, 7'd0, 1'b0}
                                             : {sign, 5'd14, 2'b00};
      end else begin
        exp_final  = 5'd15 + exp2 - 5'd1;
        frac_final = {frac1, 1'b0};
        fp4e2m1_to_fp8e5m2 = {sign, exp_final[4:0], frac_final};
      end
    end
  endfunction
  ...
  initial begin
    ...
    for (idx = 0; idx < N; idx = idx + 1) begin
      a1_fp8 = fp4e2m1_to_fp8e5m2(avec[idx][3:0]);
      a2_fp8 = fp4e2m1_to_fp8e5m2(avec[idx][7:4]);
  ...
  initial begin
    ...
    $display("\n--- FP4(E2M1) x FP8(E5M2) MAC (LAT=%0d, Vectors=%0d) ---", LAT, N);
