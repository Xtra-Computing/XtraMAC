...
function [15:0] fp8e4m3_mul_lane_to_fp16;
  input [7:0] a;
  input [7:0] b;
  reg sa,sb;
  reg [3:0] ea,eb;
  ...
  begin
    ...
    signp = sa ^ sb;
    sat_pos = FP16_MAXFIN_POS;
    sat_neg = FP16_MAXFIN_NEG;

    if (a_nan | b_nan) begin
      fp8e4m3_mul_lane_to_fp16 = FP16_QNAN;
    end else if (a_zero | b_zero) begin
      fp8e4m3_mul_lane_to_fp16 = {signp, 15'd0};
    end else begin
      es    = (ea + eb) - 7;
      Ma    = {1'b1, fa};
      Mb    = {1'b1, fb};
      ...
      if (carry) begin
        ...
        mant_pre = {prod, 4'b000};
        mant     = mant_pre[10:0];
      end

      if (es < 0 || es > 14)
        fp8e4m3_mul_lane_to_fp16 = signp ? sat_neg : sat_pos;
      else
        fp8e4m3_mul_lane_to_fp16 = {signp, (es[4:0] + 5'd8), mant[9:0]};
    end
  end
endfunction
...
initial begin : init_vectors
    // Stimulus covering normals, zeros, NaNs
    a_vec[0] = {8'h38, 8'h38};  // 1.0, 1.0
    b_vec[0] = {8'h40, 8'h38};  // 2.0, 1.0
    c_vec[0] = pack4_16(16'h0000, 16'h0000, 16'h0000, 16'h0000);

    a_vec[1] = {8'hB8, 8'h38};  // -1.0, 1.0
    b_vec[1] = {8'h38, 8'hB8};  // 1.0, -1.0
    c_vec[1] = pack4_16(16'h3C00, 16'hBC00, 16'h3C00, 16'hBC00); // +/-1.0

    a_vec[2] = {8'h00, 8'h38};  // zero, 1.0
    b_vec[2] = {8'h38, 8'h00};  // 1.0, zero
    c_vec[2] = pack4_16(16'h3C00, 16'h3C00, 16'h3C00, 16'h3C00);

    a_vec[3] = {8'h7F, 8'h38};  // qNaN, 1.0
    b_vec[3] = {8'h38, 8'h38};
    c_vec[3] = pack4_16(16'h0000, 16'h0000, 16'h0000, 16'h0000);

    a_vec[4] = {8'h38, 8'hB8};
    b_vec[4] = {8'h38, 8'h38};
    c_vec[4] = pack4_16(16'h4000, 16'hC000, 16'h4000, 16'hC000); // +/-2.0

    a_vec[5] = {8'h20, 8'h60};  // small normals
    b_vec[5] = {8'h18, 8'h28};
    c_vec[5] = pack4_16(16'h3555, 16'hB555, 16'h3A00, 16'hBA00); // ~0.333, etc.

    a_vec[6] = {8'h38, 8'h38};
    b_vec[6] = {8'h38, 8'h38};
    c_vec[6] = pack4_16(16'h7E00, 16'h0000, 16'h3C00, 16'h3C00); // c includes NaN

    a_vec[7] = {8'h38, 8'h00};
    b_vec[7] = {8'h00, 8'h38};
    c_vec[7] = pack4_16(16'h0000, 16'h3C00, 16'h0000, 16'hBC00);

    a_vec[8] = {8'h38, 8'h38};
    b_vec[8] = {8'h38, 8'h38};
    c_vec[8] = pack4_16(16'h3C00, 16'h3C00, 16'h3C00, 16'h3C00);

    a_vec[9]  = {8'hB8, 8'hB8};
    b_vec[9]  = {8'hB8, 8'h38};
    c_vec[9]  = pack4_16(16'h3C00, 16'h3C00, 16'hBC00, 16'hBC00);

    a_vec[10] = {8'h38, 8'h7F};
    b_vec[10] = {8'h7F, 8'h38};
    c_vec[10] = pack4_16(16'h3C00, 16'h3C00, 16'h3C00, 16'h3C00);

    a_vec[11] = {8'h00, 8'h00};
    b_vec[11] = {8'h00, 8'h00};
    c_vec[11] = pack4_16(16'h0000, 16'h0000, 16'h0000, 16'h0000);

    for (i = 0; i < N; i = i + 1) begin
      prod11 = fp8e4m3_mul_lane_to_fp16(a_vec[i][7:0],  b_vec[i][7:0]);
      prod12 = fp8e4m3_mul_lane_to_fp16(a_vec[i][7:0],  b_vec[i][15:8]);
      prod21 = fp8e4m3_mul_lane_to_fp16(a_vec[i][15:8], b_vec[i][7:0]);
      prod22 = fp8e4m3_mul_lane_to_fp16(a_vec[i][15:8], b_vec[i][15:8]);
  ...
  initial begin : run_test
    ...
    $display("\n--- FP8(E4M3)->FP16 MAC (LAT=%0d, Vectors=%0d) ---", LAT, N);
