...
module tb_fp8e5m2_fp16_32_mac;
  ...
  // FP8 decode (E5M2)
  wire [7:0] g_a_lo8 = a16[7:0];
  wire [7:0] g_a_hi8 = a16[15:8];

  wire        g_a_lo_sign = g_a_lo8[7];
  wire [4:0]  g_a_lo_exp  = g_a_lo8[6:2];
  wire [1:0]  g_a_lo_frac = g_a_lo8[1:0];
  wire        g_a_lo_nan  = (g_a_lo_exp == 5'h1F) && (g_a_lo_frac != 2'd0);
  wire        g_a_lo_inf  = (g_a_lo_exp == 5'h1F) && (g_a_lo_frac == 2'd0);
  wire        g_a_lo_zero = (g_a_lo_exp == 5'd0);
  wire signed [11:0] g_a_lo_exp_unbias = $signed({7'd0, g_a_lo_exp}) - 12'sd15;
  wire [2:0]  g_a_lo_mant = (g_a_lo_zero || g_a_lo_nan || g_a_lo_inf) ? 3'd0 : {1'b1, g_a_lo_frac};

  wire        g_a_hi_sign = g_a_hi8[7];
  wire [4:0]  g_a_hi_exp  = g_a_hi8[6:2];
  wire [1:0]  g_a_hi_frac = g_a_hi8[1:0];
  wire        g_a_hi_nan  = (g_a_hi_exp == 5'h1F) && (g_a_hi_frac != 2'd0);
  wire        g_a_hi_inf  = (g_a_hi_exp == 5'h1F) && (g_a_hi_frac == 2'd0);
  wire        g_a_hi_zero = (g_a_hi_exp == 5'd0);
  wire signed [11:0] g_a_hi_exp_unbias = $signed({7'd0, g_a_hi_exp}) - 12'sd15;
  wire [2:0]  g_a_hi_mant = (g_a_hi_zero || g_a_hi_nan || g_a_hi_inf) ? 3'd0 : {1'b1, g_a_hi_frac};
  ...
  reg        g_a_lo_inf_s1,  g_a_hi_inf_s1;
  reg        g_b_zero_s1, g_b_nan_s1, g_b_inf_s1;
  reg [31:0] g_c_lo_s1, g_c_hi_s1;

  always @(posedge clk) begin
    g_man_a_s1 <= {9'd0, g_a_hi_mant, 12'd0, g_a_lo_mant};
    g_man_b_s1 <= {7'd0, g_man_b_eff};

    g_exp_lo_s1 <= (g_a_lo_zero || g_a_lo_nan || g_a_lo_inf || g_b_zero || g_b_nan)
                   ? 12'sd0 : (g_a_lo_exp_unbias + ($signed({1'b0, g_b_exp}) - 12'sd15));
    g_exp_hi_s1 <= (g_a_hi_zero || g_a_hi_nan || g_a_hi_inf || g_b_zero || g_b_nan)
    ...
    g_a_lo_inf_s1  <= g_a_lo_inf;
    g_a_hi_inf_s1  <= g_a_hi_inf;
  ...
  reg        g_a_lo_inf_s2,  g_a_hi_inf_s2;
  reg        g_b_zero_s2, g_b_nan_s2, g_b_inf_s2;
  reg [31:0] g_c_lo_s2, g_c_hi_s2;
  reg [21:0] g_mul_lo_s2, g_mul_hi_s2;

  wire [13:0] g_mul_lo_raw = g_prod_w[13:0];
  wire [13:0] g_mul_hi_raw = g_prod_w[28:15];
  ...
  always @(posedge clk) begin
    ...
    g_a_lo_inf_s2  <= g_a_lo_inf_s1;
    g_a_hi_inf_s2  <= g_a_hi_inf_s1;
    ...
    g_mul_lo_s2 <= {g_mul_lo_raw, 8'd0};
    g_mul_hi_s2 <= {g_mul_hi_raw, 8'd0};
  end
  ...
  initial begin
    A_vec[0]  = 16'h3C34; B_vec[0]  = 16'h3C00; C_vec[0]  = {32'h3F800000, 32'h00000000};
    A_vec[1]  = 16'h443C; B_vec[1]  = 16'hB800; C_vec[1]  = {32'hBF800000, 32'h3F000000};
    A_vec[2]  = 16'h3C80; B_vec[2]  = 16'h4000; C_vec[2]  = {32'h3F000000, 32'h3F000000};
    A_vec[3]  = 16'hFC3C; B_vec[3]  = 16'h3C00; C_vec[3]  = {32'hBF800000, 32'h3F800000};
    A_vec[4]  = 16'h7CB8; B_vec[4]  = 16'h3C00; C_vec[4]  = 64'h0000_0000_0000_0000;
    A_vec[5]  = 16'h7DB8; B_vec[5]  = 16'h3C00; C_vec[5]  = {32'h3F800000, 32'h3F800000};
    A_vec[6]  = 16'h0101; B_vec[6]  = 16'h3C00; C_vec[6]  = 64'h0000_0000_0000_0000;
    A_vec[7]  = 16'h4848; B_vec[7]  = 16'h3E00; C_vec[7]  = {32'h3E800000, 32'hBE800000};
    A_vec[8]  = 16'h7878; B_vec[8]  = 16'h3C00; C_vec[8]  = 64'h0000_0000_0000_0000;
    A_vec[9]  = 16'h0080; B_vec[9]  = 16'hC000; C_vec[9]  = {32'h3F800000, 32'hBF800000};
    A_vec[10] = 16'h7C00; B_vec[10] = 16'h0000; C_vec[10] = 64'h0000_0000_0000_0000;
    A_vec[11] = 16'h3C12; B_vec[11] = 16'h7E01; C_vec[11] = 64'h0000_0000_0000_0000;
    A_vec[12] = 16'h2C34; B_vec[12] = 16'h7C00; C_vec[12] = 64'h0000_0000_0000_0000;
    A_vec[13] = 16'h7DFF; B_vec[13] = 16'h3C00; C_vec[13] = {32'h3F800000, 32'hBF800000};
  end
