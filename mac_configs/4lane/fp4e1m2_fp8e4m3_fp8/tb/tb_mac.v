...
// =============================================================
// Auto-generated TB for fp4e1m2_fp8e4m3_fp8 (lane=4lane)
...
module tb_mac;
  ...
  fp4e1m2_fp8e4m3_fp8_mac_4c u4 (.clk(clk), .a_fp4(a_fp4), .b_fp8(b_fp8), .c_fp8(c_fp8), .result(y4));
  fp4e1m2_fp8e4m3_fp8_mac_5c u5 (.clk(clk), .a_fp4(a_fp4), .b_fp8(b_fp8), .c_fp8(c_fp8), .result(y5));
  fp4e1m2_fp8e4m3_fp8_mac_6c u6 (.clk(clk), .a_fp4(a_fp4), .b_fp8(b_fp8), .c_fp8(c_fp8), .result(y6));
  ...
  initial begin
    ...
    $display("CONFIG=fp4e1m2_fp8e4m3_fp8 LANE=4lane EXP=%0d,%0d,%0d MEAS=%0d,%0d,%0d CMP=%0d ERR5=%0d ERR6=%0d",
             EXP4, EXP5, EXP6, lat4_m, lat5_m, lat6_m,
             compared, errors_5, errors_6);

    if (lat4_m == EXP4 && lat5_m == EXP5 && lat6_m == EXP6 &&
        errors_5 == 0 && errors_6 == 0)
      $display("RESULT=fp4e1m2_fp8e4m3_fp8 PASS");
    else
      $display("RESULT=fp4e1m2_fp8e4m3_fp8 FAIL");
    $finish;
  end
  ...
  initial begin
    #2000000;
    $display("RESULT=fp4e1m2_fp8e4m3_fp8 FAIL (timeout)");
    $finish;
  end
